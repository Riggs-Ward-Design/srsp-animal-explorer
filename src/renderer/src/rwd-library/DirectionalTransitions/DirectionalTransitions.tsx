...
type VariantsByType = Record<TransitionType, VariantSet>

const createVariantsByType = (options: TransitionOptions | undefined): VariantsByType => {
  //
...
const DirectionalTransitions = ({
  ...
  currentPosition,
  options
}: DirectionalTransitionsProps): ReactElement => {
  ...
  const [prevPosition, setPrevPosition] = useState<Coords3D>(currentPosition)
  const [transitionType, setTransitionType] = useState<TransitionType>('fade')

  const posKey = `${currentPosition.x},${currentPosition.y},${currentPosition.z}`
  const prevKey = `${prevPosition.x},${prevPosition.y},${prevPosition.z}`

  if (posKey !== prevKey) {
    const newType = getTransitionTypeFromPositions(prevPosition, currentPosition)
