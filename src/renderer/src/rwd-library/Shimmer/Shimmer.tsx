...
function Shimmer(props: ShimmerProps): ReactElement {
  //
  ...
  const [keyframeName] = useState(() => `shimmer-${Math.random().toString(36).slice(2, 11)}`)
  ...
  return (
    <>
      ...
      <span style={containerStyle} onAnimationIteration={() => setIterations((i) => i + 1)}>
        {props.children}
      </span>
    </>
  )
